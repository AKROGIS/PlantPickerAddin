...
{
    /// <summary>
    /// A combo box for selecting a plant species and adding an abundance layer for that species
    /// to the current map.
    /// </summary>
    internal class SpeciesCoverage : ComboBox
    {
        /// <summary>
        ...
        public SpeciesCoverage()
        {
            _layerBuilder = new SpeciesLayerFactory("Plant Cover by Species.lyrx")
            {
                ...
                CIMFixer = LayerUtilities.RandomizeMarkerColor,
            };
            Task.Run(InitAsync);
        }
